...
use unicornafl::{Permission, HookType};
...
fn main() {
    ...
    emu.reg_write(RegisterARM::SP as i32, STACK_ADDR as u64 - 0x24).expect("failed write SP"); 
    ...
    emu.mem_map(0x70055000 as u64, 4096, Protection::READ | Protection::WRITE).expect("failed to map map current task index");
    emu.mem_map(0xF7330000, 4096, Protection::READ | Protection::WRITE).expect("failed to map active module id");
    emu.mem_map(0xF6D05000, 4096*3, Protection::READ | Protection::WRITE).expect("failed to map segment for global RecvMsg objects");
    emu.mem_map(0xF6E3A000, 4096, Protection::READ | Protection::WRITE).expect("failed to map area for mcd_unpack");
    ...
    */ 
    ...
    let msg_recv = |mut uc: Unicorn, _addr: u64, _size: u32| {
        ...
        let mut buf_len_raw: [u8; 4] = [0; 4 as usize];
        ...
        if buf_len < 4 {
            ...
            uc.mem_write(ilm_ptr, b"\x00\x01").expect("failed to prepare ilm_struct inside msg_recv_*q"); // src_mod_id
            uc.mem_write(ilm_ptr + 2, b"\xb5\x01").expect("failed to prepare ilm_struct inside msg_recv_*q"); // dest_mod_id
            uc.mem_write(ilm_ptr + 4, b"\x00\x00").expect("failed to prepare ilm_struct inside msg_recv_*q"); // sap_id
            // msg_id
            uc.mem_write(ilm_ptr + 6, b"\x0c\x55").expect("failed to prepare ilm_struct inside msg_recv_*q"); // rcv_attach_accept_ind
        
            // construct local_para_struct
            let local_para_ptr = uc_alloc(&mut uc, 4 + 4 + 4).expect("failed to alloc");
            uc.mem_write(local_para_ptr, b"\x01").expect("failed to write local_para->ref_count");
            // size = header + length_u32 + pdu_ptr
            uc.mem_write(local_para_ptr + 2, &((4 + 4 + 4) as u16).to_le_bytes()).expect("failed to write local_para->msg_len");
            uc.mem_write(local_para_ptr + 4, &((buf_len as u32).to_le_bytes())).expect("failed to write local_para->payload");
            uc.mem_write(local_para_ptr + 8, &((0x0A000008 as u32).to_le_bytes())).expect("failed to write local_para->payload");
            
            uc.mem_write(ilm_ptr + 8, &((local_para_ptr as u32).to_le_bytes())).expect("failed to prepare ilm_struct->local_para_ptr inside msg_recv_*q");
            
            let null_ptr: [u8; 4] = [0; 4 as usize];
            uc.mem_write(ilm_ptr + 12, &null_ptr).expect("failed to prepare ilm_struct->peer_buff_ptr inside msg_recv_*q");
    ...
    let free_ctrl_buffer_ext = |mut uc: Unicorn, _addr: u64, _size: u32| {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            unsafe { 
                ...
                println!("[*] free_ctrl_buffer_ext: addr {:#010x}; {}:{}\n", buff_ptr, file_name, line);
            }
        }
    ...
    let cemm_int_state8get_state_ev = |mut uc: Unicorn, _addr: u64, _size: u32| {
        uc.reg_write(RegisterARM::R0 as i32, 0x4).expect("failed to write pc inside CEmmIntState8getStateEv");
        let lr = uc.reg_read(RegisterARM::LR as i32).expect("failed to read LR");
        uc.reg_write(RegisterARM::PC as i32, lr).expect("failed to write pc inside CEmmIntState8getStateEv");
    };
    ...
    let mcd_unpack = |mut uc: Unicorn, _addr: u64, _size: u32| {
        ...
        uc.mem_read(0x0A000000, &mut buf_len_raw).expect("failed to read length of input buffer");
        let buf_len = u32::from_le_bytes(buf_len_raw);
        if buf_len < 8 || buf_len > 255 {
            uc.reg_write(RegisterARM::R0 as i32, 0xffffffff).expect("failed to write return val inside mcd_unpack");
        } else {
            let new_buf_addr = uc_alloc(&mut uc, buf_len as u64).expect("failed to alloc");
            let mut new_buf = vec![0; buf_len as usize];
            uc.mem_read(0x0A000008, &mut new_buf).expect("failed to read input buf");
            uc.mem_write(new_buf_addr, &new_buf).expect("failed to write input buf into newly allocated mem");

            let dest = uc.reg_read(RegisterARM::R1 as i32).expect("failed to read r1");
            uc.mem_write(dest + 0x05, b"\x01").expect("failed to write buf to dest");
            uc.mem_write(dest + 0x08, b"\x07").expect("failed to write buf to dest");
            uc.mem_write(dest + 0x40, b"\x01").expect("failed to write buf to dest");
            uc.mem_write(dest + 0x41, &(buf_len as u8).to_le_bytes()).expect("failed to write buf to dest");
            uc.mem_write(dest + 0x44, &(new_buf_addr).to_le_bytes()).expect("failed to write buf to dest");
            uc.reg_write(RegisterARM::R0 as i32, (buf_len) as u64).expect("failed to write return val inside mcd_unpack");
        }
        let lr = uc.reg_read(RegisterARM::LR as i32).expect("failed to read LR");
        uc.reg_write(RegisterARM::PC as i32, lr).expect("failed to write back PC");
    };
    
    let init =|mut uc: Unicorn, _addr: u64, _size: u32| {
        uc.reg_write(RegisterARM::R0 as i32, 0xF6D06F5C).expect("failed to write _this_ reference to r0 for decodeCheckAttachAccept");
        let internal_buf = uc_alloc(&mut uc, 0x47c).expect("failed to alloc");
        uc.reg_write(RegisterARM::R1 as i32, internal_buf).expect("failed to write internal_buf pointer to r1 for decodeCheckAttachAccept");
        uc.reg_write(RegisterARM::R2 as i32, 0x0).expect("failed to write pdu reference to r2 for decodeCheckAttachAccept"); // we don't need the pdu
        uc.reg_write(RegisterARM::R3 as i32, 0x0).expect("failed to write pdu size reference to r3 for decodeCheckAttachAccept"); // hence we don't need the size
        let decoded_buf = uc_alloc(&mut uc, 0x800).expect("failed to alloc");
        uc.reg_write(RegisterARM::R4 as i32, decoded_buf).expect("failed to write decoded_buf pointer to r4 for decodeCheckAttachAccept"); 
        uc.reg_write(RegisterARM::R5 as i32, 0x800).expect("failed to write decode_buff size reference to r5 for decodeCheckAttachAccept");        
    };
    ...
    macro_rules! hook {
        ($addr:expr, $func:expr) => {
            emu.add_code_hook(HookType::CODE, $addr, $addr, Box::new($func)).expect(&format!("failed to set {} hook", stringify!($func)));
        };
        ($addr:expr, $func:expr, $opt_name:expr) => {
            emu.add_code_hook(HookType::CODE, $addr, $addr, Box::new($func)).expect(&format!("failed to set {} hook", $opt_name));
        };
    }
    ...
    let place_input_callback = |mut uc: Unicorn, afl_input :&[u8], _:i32| {
        uc.mem_write(0x0A000000, &(afl_input.len() as u32).to_le_bytes()).expect("failed to write input_size");
        uc.mem_write(0x0A000000 + 8, &afl_input).expect("failed to write input buffer");
        true
    };

    let crash_validation_callback = | _uc: Unicorn, result: unicornafl::ffi::uc_error, _input: &[u8], _:i32 | {
    ...
    emu.emu_start(0x001e6a31, 0x001e6c82, 0, 1).expect("failed to kick off"); // start at offset 1 to run in thumb mode
