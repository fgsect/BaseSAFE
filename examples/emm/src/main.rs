...
fn main() {
    ...
    emu.reg_write(Register::SP as i32, STACK_ADDR as u64 - 0x24).expect("failed write SP"); 
    ...
    */ 
    ...
    let cemm_int_state8get_state_ev = |mut uc: Unicorn, _addr: u64, _size: u32| {
        uc.reg_write(Register::R0 as i32, 0x4).expect("failed to write pc inside CEmmIntState8getStateEv");
        let lr = uc.reg_read(Register::LR as i32).expect("failed to read LR");
        uc.reg_write(Register::PC as i32, lr).expect("failed to write pc inside CEmmIntState8getStateEv");
    };
    ...
    let mcd_unpack = |mut uc: Unicorn, _addr: u64, _size: u32| {
        ...
        if buf_len < 8 || buf_len > 255 {
            uc.reg_write(Register::R0 as i32, 0xffffffff).expect("failed to write return val inside mcd_unpack");
        } else {
            ...
            let dest = uc.reg_read(Register::R1 as i32).expect("failed to read r1");
            ...
            uc.reg_write(Register::R0 as i32, (buf_len) as u64).expect("failed to write return val inside mcd_unpack");
        }
        let lr = uc.reg_read(Register::LR as i32).expect("failed to read LR");
        uc.reg_write(Register::PC as i32, lr).expect("failed to write back PC");
    };
    
    let init =|mut uc: Unicorn, _addr: u64, _size: u32| {
        uc.reg_write(Register::R0 as i32, 0xF6D06F5C).expect("failed to write _this_ reference to r0 for decodeCheckAttachAccept");
        let internal_buf = uc_alloc(&mut uc, 0x47c).expect("failed to alloc");
        uc.reg_write(Register::R1 as i32, internal_buf).expect("failed to write internal_buf pointer to r1 for decodeCheckAttachAccept");
        uc.reg_write(Register::R2 as i32, 0x0).expect("failed to write pdu reference to r2 for decodeCheckAttachAccept"); // we don't need the pdu
        uc.reg_write(Register::R3 as i32, 0x0).expect("failed to write pdu size reference to r3 for decodeCheckAttachAccept"); // hence we don't need the size
        let decoded_buf = uc_alloc(&mut uc, 0x800).expect("failed to alloc");
        uc.reg_write(Register::R4 as i32, decoded_buf).expect("failed to write decoded_buf pointer to r4 for decodeCheckAttachAccept"); 
        uc.reg_write(Register::R5 as i32, 0x800).expect("failed to write decode_buff size reference to r5 for decodeCheckAttachAccept");        
    };
