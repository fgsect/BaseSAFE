...
fn main() {
    ...
    */ 
    ...
    let msg_recv = move |mut uc: Unicorn, _addr: u64, _size: u32| {
        ...
        if buf_len < 4 || buf_len == 65535 {
            ...
            if msg_id == 4 { // DL_DCCH with pdu in peer_buff
                ...
                uc.mem_write(peer_buff_ptr + 6, &((free_tail_space[0] as u16).to_le_bytes())).expect("failed to write peer_buff_buf->free_tail_space");
                uc.mem_write(peer_buff_ptr + 8 + free_header_space[0] as u64, &new_buf).expect("failed to write peer_buff_buf->payload");

                uc.mem_write(ilm_ptr + 12, &((peer_buff_ptr as u32).to_le_bytes())).expect("failed to prepare ilm_struct->peer_buff_ptr inside msg_recv_*q");
                uc.mem_write(ilm_ptr + 8, &null_ptr).expect("failed to prepare ilm_struct->local_para_ptr inside msg_recv_*q");
            } else {
                ...
                let local_para_ptr = uc_alloc(&mut uc, 4 + 0x10 + 8 + 4 + 4).expect("failed to alloc");
                uc.mem_write(local_para_ptr, b"\x01").expect("failed to write local_para->ref_count");
                uc.mem_write(local_para_ptr + 2, &((4 + 0x10 + 8 + 4 + 4) as u16).to_le_bytes()).expect("failed to write local_para->msg_len");

                if msg_id == 0 || mode.to_ascii_uppercase() == "PCCH" {
                    msg_id = 0x5082;
                    uc.mem_write(local_para_ptr + 8, &((qbm_ptr as u32).to_le_bytes())).expect("failed to write local_para->payload");
                } else if msg_id == 1 || mode.to_ascii_uppercase() == "BCCH_DL_SCH" {
                    msg_id = 0x5078;
                    uc.mem_write(local_para_ptr + 8, &((qbm_ptr as u32).to_le_bytes())).expect("failed to write local_para->payload");
                } else if msg_id == 2 || mode.to_ascii_uppercase() == "DL_CCCH" {
                    msg_id = 0x5086;
                    uc.mem_write(local_para_ptr + 4, &((qbm_ptr as u32).to_le_bytes())).expect("failed to write local_para->payload");
                } else if msg_id == 3 || mode.to_ascii_uppercase() == "DL_DCCH" {
                    msg_id = 0x5094;
                    uc.mem_write(local_para_ptr + 0x10, &((qbm_ptr as u32).to_le_bytes())).expect("failed to write local_para->payload");
                }
    ...
    hook!(0x001f4368, pass_func, "errc_com_calculate_procedure_delay_start");
    ...
    let place_input_callback = |mut uc: Unicorn, afl_input :&[u8], _:i32| {
        if afl_input.len() > 4096 {
            ...
            uc.mem_write(0x0A000000, &(afl_input.len() as u16).to_le_bytes()).expect("failed to write input_size");
    ...
    emu.emu_start(0x1fe741, 0x001ff106, 0, 1).expect("failed to kick off emulation"); // start at offset 1 to run in thumb mode
